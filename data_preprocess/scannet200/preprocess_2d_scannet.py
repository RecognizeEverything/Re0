...
parser.add_argument('--only_validation',  action='store_true', help='only need validation')
...
print(opt)
validation_scenes=[]
if opt.only_validation:
    with open('scannetv2_val.txt', 'r') as file:
        # 逐行读取并转换为数字，存储到列表中
        lines = [str(line.strip()) for line in file]
    validation_scenes = lines
...
def main():
    ...
    for i in trange(0,len(scenes)):
        ...
        point_cloud_path = os.path.join(opt.scannet_path, scenes[i], scenes[i] + '_vh_clean_2.ply')
        print(point_cloud_path + ' copy to ' + os.path.join(opt.output_path, scenes[i], scenes[i] + '_vh_clean_2.ply'))
        os.makedirs(os.path.join(opt.output_path, scenes[i]), exist_ok=True)
        shutil.copy(point_cloud_path, os.path.join(opt.output_path, scenes[i], scenes[i] + '_vh_clean_2.ply'))
